...
with open(name) as csv_file:
    ...
    with open(name) as csv_file:
        ...
        for row in csv_reader:
            ...
            wt_func = ramp_up_weight(line_count)
            wt = next(wt_func)
            ...
            graph_arr[pz_unsup_loss][line_count - 1] = float(row["pz_unsup_dice_loss"]) * wt
            graph_arr[cz_unsup_loss][line_count - 1] = float(row["cz_unsup_dice_loss"]) * wt
            graph_arr[us_unsup_loss][line_count - 1] = float(row["us_unsup_dice_loss"]) * wt * 0.5
            graph_arr[afs_unsup_loss][line_count - 1] = float(row["afs_unsup_dice_loss"]) * wt * 0.5
            graph_arr[bg_unsup_loss][line_count - 1] = float(row["bg_unsup_dice_loss"]) * wt
            ...
            graph_arr[train_loss][line_count - 1] = \
                graph_arr[pz_dice_loss][line_count - 1] + graph_arr[cz_dice_loss][line_count - 1] + \
                graph_arr[us_dice_loss][line_count - 1] + graph_arr[afs_dice_loss][line_count - 1] + \
                graph_arr[bg_dice_loss][line_count - 1]
            + graph_arr[pz_unsup_loss][line_count - 1] + graph_arr[cz_unsup_loss][line_count - 1] + \
            graph_arr[us_unsup_loss][line_count - 1] + graph_arr[afs_unsup_loss][line_count - 1] + \
            graph_arr[bg_unsup_loss][line_count - 1]
