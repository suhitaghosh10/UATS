...
class weighted_model:
    ...
    def build_model(self, img_shape=(32, 168, 168), use_dice_cl=None, num_class=5, learning_rate=5e-5, gpu_id=None,
        ...
        temp_inp = Input((1,), name="temp_inp")
        # cons = K.constant(K.slice(temp,0,1))

        temperature = Lambda(lambda x: x[0, 0], name='temperature_cons')(temp_inp)
        # cons = K.constant(cons)
        # cons =  tf.Variable(temp, expected_shape=(1,), validate_shape=(1,) ,dtype=tf.float32, trainable=False)
        # cons =  tf.Variable(temp, expected_shape=(1,), validate_shape=(1,) ,dtype=tf.float32, trainable=False)
        ...
        if (nb_gpus is None):
            p_model = Model([input_img, unsupervised_label, supervised_flag, temp_inp],
            ...
            with tf.device(gpu_id):
                model = Model([input_img, unsupervised_label, supervised_flag, temp_inp],
                              [pz_sm_out, cz_sm_out, us_sm_out, afs_sm_out, bg_sm_out])
                if trained_model is not None:
