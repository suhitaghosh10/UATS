...
TB_LOG_DIR = './tensor_temporal_ramp_max'
...
def train(train_x, train_y, val_x, val_y):
    ...
    class TemporalCallback(Callback):
        ...
        def on_batch_end(self, batch, logs=None):
            ...
            if batch == self.last_batch_no:
                ...
                self.unsupervised_weight = np.where(cur_pred < max, max, cur_pred)
                next_weight = next(gen_weight)
                # update ensemble_prediction and unsupervised weight when an epoch ends
                if self.epoch == 0:
                    max = np.reshape(np.max(cur_pred, axis=-1), (num_train_data, 32, 168, 168, 1))
                    self.unsupervised_weight = np.where(cur_pred < max, max * next_weight, cur_pred)
                else:
                    max = np.reshape(np.max(self.ensemble_prediction, axis=-1), (num_train_data, 32, 168, 168, 1))
                    self.unsupervised_weight = np.where(self.ensemble_prediction < max, max, self.ensemble_prediction)
    ...
    model_checkpoint = ModelCheckpoint('./temporal_max_ramp.h5', monitor='val_loss', save_best_only=True, verbose=1,
                                       mode='min')
