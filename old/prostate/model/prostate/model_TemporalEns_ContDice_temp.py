...
class weighted_model:
    ...
    def unsup_dice_tb(self, input, class_wt=1., alpha=0.6):

        def unsup_dice_loss(y_true, y_pred, smooth=1., axis=(1, 2, 3)):

            y_true = input[:, :, :, :, 0]
            y_true = y_true / (1 - alpha ** (self.epoch_ctr + 1))
    ...
    def build_model(self, img_shape=(32, 168, 168), use_dice_cl=None, num_class=5, learning_rate=5e-5, gpu_id=None,
        ...
        conv_out = Lambda(lambda x: x / 2)(conv_out)
        conv_out = Activation('softmax')(conv_out)
        ...
        pz = concatenate([pz_ensemble_pred, supervised_flag], name='pz_c')
        cz = concatenate([cz_ensemble_pred, supervised_flag], name='cz_c')
        us = concatenate([us_ensemble_pred, supervised_flag], name='us_c')
        afs = concatenate([afs_ensemble_pred, supervised_flag], name='afs_c')
        bg = concatenate([bg_ensemble_pred, supervised_flag], name='bg_c')
