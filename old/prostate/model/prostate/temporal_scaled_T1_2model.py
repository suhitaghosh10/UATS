...
class weighted_model:
    ...
    def build_model(self, img_shape=(32, 168, 168), use_dice_cl=None, num_class=5, learning_rate=5e-5, gpu_id=None,
        ...
        conv_out2 = Lambda(lambda x: x / temp, name='scaling')(conv_out)

        conv_out_sm = Activation('softmax')(conv_out)
        conv_out_sm_sc = Activation('softmax')(conv_out2)

        pz_sm_out = Lambda(lambda x: x[:, :, :, :, 0], name='pz_n')(conv_out_sm)
        cz_sm_out = Lambda(lambda x: x[:, :, :, :, 1], name='cz_n')(conv_out_sm)
        us_sm_out = Lambda(lambda x: x[:, :, :, :, 2], name='us_n')(conv_out_sm)
        afs_sm_out = Lambda(lambda x: x[:, :, :, :, 3], name='afs_n')(conv_out_sm)
        bg_sm_out = Lambda(lambda x: x[:, :, :, :, 4], name='bg_n')(conv_out_sm)

        pz_sm_out_sc = Lambda(lambda x: x[:, :, :, :, 0], name='pz')(conv_out_sm_sc)
        cz_sm_out_sc = Lambda(lambda x: x[:, :, :, :, 1], name='cz')(conv_out_sm_sc)
        us_sm_out_sc = Lambda(lambda x: x[:, :, :, :, 2], name='us')(conv_out_sm_sc)
        afs_sm_out_sc = Lambda(lambda x: x[:, :, :, :, 3], name='afs')(conv_out_sm_sc)
        bg_sm_out_sc = Lambda(lambda x: x[:, :, :, :, 4], name='bg')(conv_out_sm_sc)
        ...
        if (nb_gpus is None):
            ...
            sc_model = Model([input_img, unsupervised_label, supervised_flag],
                             [pz_sm_out_sc, cz_sm_out_sc, us_sm_out_sc, afs_sm_out_sc, bg_sm_out_sc])
            if trained_model is not None:
                sc_model.load_weights(trained_model, by_name=True)
            weights_list = sc_model.get_weights()

            p_model = Model([input_img, unsupervised_label, supervised_flag],
                            [pz_sm_out, cz_sm_out, us_sm_out, afs_sm_out, bg_sm_out])
            p_model.set_weights(weights_list)
