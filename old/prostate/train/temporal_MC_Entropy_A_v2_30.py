...
from prostate.generator.temporal_A import DataGenerator
...
FOLD_NUM = 2
TRAIN_NUM = 30
NAME = 'Entropy_F' + str(FOLD_NUM) + '_' + str(TRAIN_NUM)
...
ENS_GT_PATH = '/data/suhita/temporal/sadv6/ens_gt/'
FLAG_PATH = '/data/suhita/temporal/sadv6/flag/'
...
def train(gpu_id, nb_gpus):
    ...
    wm = weighted_model()
    model, p_model_MC, normal_model = wm.build_model(num_class=NUM_CLASS, learning_rate=learning_rate, gpu_id=gpu_id,
            ...
            if epoch > 0:
                ...
                for b_no in np.arange(num_batches):
                    ...
                    if True:
                        ...
                        flag = np.reshape(entropy_zone, (patients_per_batch, 32, 168, 168))
...
def predict(val_x, val_y, model_name, eval=True, out_npy=None):
    val_imgs = np.load(val_x)

    wm = weighted_model()
    model = wm.build_model(learning_rate=learning_rate, gpu_id=None,
                           nb_gpus=None, trained_model=model_name)
    print('load_weights')
    val_y_arr = np.load(val_y)
    val_y_arr = val_y_arr.astype(np.uint8)
    val_supervised_flag = np.ones((val_y_arr.shape[0], 32, 168, 168), dtype='int8')
    pz = val_y_arr[:, :, :, :, 0]
    cz = val_y_arr[:, :, :, :, 1]
    us = val_y_arr[:, :, :, :, 2]
    afs = val_y_arr[:, :, :, :, 3]
    ...
    y_val = [pz, cz, us, afs, bg]
    x_val = [val_imgs, val_y_arr, val_supervised_flag]
    out = model[0].predict(x_val, batch_size=1, verbose=1)
    np.save(os.path.join(out_npy), out)
    if eval:
        scores = model[0].evaluate(x_val, y_val, batch_size=2, verbose=1)
        for i in range(len(model[0].metrics_names)):
            print("%s: %.16f%%" % (model[0].metrics_names[i], scores[i]))
...
if __name__ == '__main__':
    ...
    val_x = '/cache/suhita/data/final_test_array_imgs.npy'
    val_y = '/cache/suhita/data/final_test_array_GT.npy'
    predict(val_x, val_y, '/data/suhita/temporal/Entropy_F2_30.h5', eval=True, out_npy='/data/suhita/temporal/p.npy')
