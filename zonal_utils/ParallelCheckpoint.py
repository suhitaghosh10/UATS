...
class ModelCheckpointParallel(keras.callbacks.Callback):
    ...
    Save the model_impl after every epoch.
    `filepath` can contain named formatting options,
    which will be filled the value of `epoch` and
    keys in `logs` (passed in `on_epoch_end`).
    For example: if `filepath` is `weights.{epoch:02d}-{val_loss:.2f}.hdf5`,
    then the model_impl checkpoints will be saved with the epoch number and
    the validation loss in the filename.
    # Arguments
        filepath: string, path to save the model_impl file.
        monitor: quantity to monitor.
        verbose: verbosity mode, 0 or 1.
        save_best_only: if `save_best_only=True`,
            the latest best model_impl according to
        ...
        save_weights_only: if True, then only the model_impl's weights will be
            saved (`model_impl.save_weights(filepath)`), else the full model_impl
            is saved (`model_impl.save(filepath)`).
    ...
    def on_batch_end(self, batch, logs=None):
        if self.at_batch_end is not None and np.mod(batch + 1, self.at_batch_end) == 0:
            print("Saving model_impl at batch end!")
    ...
    def on_model_save(self, epoch, iter, logs=None):
        """ save the model_impl to hdf5. Code mostly from keras core """
        ...
        if self.epochs_since_last_save >= self.period:
            ...
            if self.save_best_only:
                ...
                if current is None:
                    warnings.warn('Can save best model_impl only with %s available, '
                                  'skipping.' % (self.monitor), RuntimeWarning)
                else:
                    if self.monitor_op(current, self.best):
                        if self.verbose > 0:
                            print('Epoch %05d: Iter%05d: %s improved from %0.5f to %0.5f,'
                                  ' saving model_impl to %s'
                ...
                if self.verbose > 0:
                    print('Epoch %05d: saving model_impl to %s' % (epoch, filepath))
                if self.save_weights_only:
                    self.model.layers[-(num_outputs + 1)].save_weights(filepath, overwrite=True)
                else:
                    self.model.layers[-(num_outputs + 1)].save(filepath, overwrite=True)
