...
TB_LOG_DIR = './tb/variance_mcdropout/dice'
MODEL_NAME = './temporal_dice.h5'
UPDATE_WTS_AFTER_EPOCH = 1
ENSEMBLE_NO = 10


def train(train_x_inp, train_y_inp, train_ux_inp, train_ux_predicted_inp, val_x, val_y, gpu_id, nb_gpus):
    ...
    # Build Model
    model, model_no_sm = build_model(num_class=num_class, use_dice_cl=True, learning_rate=learning_rate, gpu_id=gpu_id,
                                     nb_gpus=nb_gpus)
    ret_dic = make_dataset(train_x_inp, train_y_inp, train_ux_inp, train_ux_predicted_inp, val_x, val_y, num_class,
                           model)
    ...
    class TemporalCallback(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            if epoch > UPDATE_WTS_AFTER_EPOCH - 1:
                ...
                self.prev_output_b4_softmax = (cur_epoch_model_pred - np.amin(cur_epoch_model_pred)) / (
                            np.amax(cur_epoch_model_pred) - np.amin(cur_epoch_model_pred))
                del cur_epoch_model_pred
            ...
            if epoch > UPDATE_WTS_AFTER_EPOCH:
                ...
                # cur_pred_final = np.where(cur_pred == max and cur_pred>= DICE_THRESHOLD, 1., 0.)
                #del cur_pred
    ...
    print('Fitting model...')
    print('-' * 30)
    training_generator = DataGenerator(imgs, unsupervised_target, supervised_label, supervised_flag,
...
if __name__ == '__main__':
    ...
    train_ux = np.load('/home/suhita/zonals/data/training/trainArray_unlabeled_imgs_fold1.npy')
    train_ux_predicted = np.load('/home/suhita/zonals/data/training/trainArray_unlabeled_GT_fold1.npy')
