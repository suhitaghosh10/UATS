...
from keras.layers import concatenate, Input, Conv2D, MaxPooling2D, Conv2DTranspose, BatchNormalization, Dropout
...
class weighted_model:
    ...
    def build_model(self, img_shape=(32, 168, 168), learning_rate=5e-5, gpu_id=None,
        ...
        conv_out_sm = Conv2D(1, (1, 1), activation='sigmoid', name='conv_final')(conv7)

        skin = K.stack([unsupervised_label, supervised_flag])
        ...
        if (nb_gpus is None):
            model.compile(optimizer=optimizer,
                          loss=self.semi_supervised_loss(skin, unsup_loss_class_wt=1)
                          ,
            ...
            with tf.device(gpu_id):
                merged_model = multi_gpu_model(model, gpus=nb_gpus)

                model.compile(optimizer=optimizer,
                              loss=self.semi_supervised_loss(skin, unsup_loss_class_wt=1)
                              ,
                              metrics={'kidney': [self.dice_coef, self.unsup_dice_tb(skin, 1),
                                                  self.dice_tb(skin, 1)]
                                       }
                              )
        ...
        conv_out_mc = Conv2D(1, (1, 1), activation='sigmoid', name='conv_final_softmax_mc')(conv7)
