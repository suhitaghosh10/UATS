...
ENS_GT_PATH = '/data/suhita/temporal/skin/output/mc/sadv1/'
...
'''
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            if epoch > 0:
                ...
                for b_no in np.arange(num_batches):
                    ...
                    save_array(self.ensemble_path, flag, start, end)
    ...
    val_GT_arr = np.zeros((num_val_data, DIM[0], DIM[1], 1), dtype=float)
    ...
    for i in np.arange(num_val_data):
        ...
        val_GT_arr[i] = np.load(os.path.join(VAL_DATA, 'GT', val_fold[i]).replace('.npy', '_segmentation.npy'))

    x_val = [val_img_arr / 255, val_GT_arr / 255, val_supervised_flag]
    y_val = val_GT_arr / 255
