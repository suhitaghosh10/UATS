...
class weighted_model:
    ...
    def upLayer(self, inputLayer, concatLayer, filterSize, i, bn=False, do=False):
        up = Conv3DTranspose(filterSize, (2, 2, 2), strides=(2, 2, 2), activation='relu', padding='same',
    ...
    def build_model(self, img_shape=(32, 168, 168), learning_rate=5e-5):
        ...
        conv_out = Conv3D(3, (1, 1, 1), activation='softmax', name='conv_final_softmax')(conv7)
        bg = Lambda(lambda x: x[:, :, :, :, 0], name='bg')(conv_out)
        z1 = Lambda(lambda x: x[:, :, :, :, 1], name='z1')(conv_out)
        z2 = Lambda(lambda x: x[:, :, :, :, 2], name='z2')(conv_out)
        ...
        p_model = Model(input_img, [bg, z1, z2])
        p_model.compile(optimizer=optimizer, loss={'bg': self.dice_loss,
                                                   'z1': self.dice_loss,
                                                   'z2': self.dice_loss},
                        metrics={'bg': self.dice_coef, 'z1': self.dice_coef, 'z2': self.dice_coef})
