...
class weighted_model:
    ...
    ramp_up_period = 200
    ramp_up_wt = K.variable(0, name='ramp_up_wt')
    ...
    class LossCallback(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            ramp_up_wt = ramp_up_weight(weighted_model.ramp_up_period, 5., epoch)
            # print(ramp_up_wt)
    ...
    def semi_supervised_loss(self, input, unsup_loss_class_wt=1., alpha=0.6):
        ...
        def loss_func(y_true, y_pred):
            ...
            print(weighted_model.ramp_up_wt)
            return supervised_loss + (weighted_model.ramp_up_wt * unsup_loss_class_wt * unsupervised_loss)
    ...
    def upLayer_MC(self, inputLayer, concatLayer, filterSize, i, bn=False, do=False):
        ...
        if bn:
            conv = BatchNormalization()(conv)
        if do:
            conv = Dropout(0.5, seed=3, name='Dropout_mc' + str(i))(conv)
        conv = Conv3D(int(filterSize / 2), (3, 3, 3), activation='relu', padding='same',
    ...
    def build_model(self, img_shape=(32, 168, 168), use_dice_cl=None, num_class=5, learning_rate=5e-5, gpu_id=None,
        ...
        if bn:
            conv4 = BatchNormalization()(conv4)
        if do:
            conv4 = Dropout(0.5, seed=4, name='Dropout_mc' + str(4))(conv4)
        conv4 = Conv3D(sfs * 16, (3, 3, 3), activation='relu', padding='same', kernel_initializer=kernel_init,
        ...
        if bn:
            conv5 = BatchNormalization()(conv5)
        if do:
            conv5 = Dropout(0.5, seed=5, name='Dropout_mc' + str(5))(conv5)
        conv5 = Conv3D(int(sfs * 8), (3, 3, 3), activation='relu', padding='same', kernel_initializer=kernel_init,
        ...
        pz = K.stack([pz_sm_out_mc, supervised_flag])
        cz = K.stack([cz_sm_out_mc, supervised_flag])
        us = K.stack([us_sm_out_mc, supervised_flag])
        afs = K.stack([afs_sm_out_mc, supervised_flag])
        bg = K.stack([bg_sm_out_mc, supervised_flag])
                ...
                merged_model.compile(optimizer=optimizer,
                                     ...
                                     metrics={'pz': [self.dice_coef, self.unsup_dice_tb(pz, 1), self.dice_tb(pz, 1)],
                                              ...
                                              'afs': [self.dice_coef, self.unsup_dice_tb(afs, 2),
                                                      self.dice_tb(afs, 2 * 2)],
                                              'bg': [self.dice_coef, self.unsup_dice_tb(bg, 1), self.dice_tb(bg, 1)]
                                              },
                                     loss_weights={'pz': 1, 'cz': 1, 'us': 2, 'afs': 2, 'bg': 1}
                                     )

        return merged_model, p_model_normal, p_model_MC
