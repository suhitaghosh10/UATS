...
class weighted_model:
    ...
    def build_model(self, img_shape=(152, 152, 56), learning_rate=5e-5, gpu_id=None,
                    nb_gpus=None,
        ...
        conv_out = Conv3D(1, (1, 1, 1), name='conv_final_softmax')(conv7)
        conv_out = Lambda(lambda x: x / temp)(conv_out)
        conv_out_sm = Activation('sigmoid')(conv_out)

        kidney = K.stack([unsupervised_label, supervised_flag])

        # optimizer = AdamWithWeightnorm(lr=learning_rate, beta_1=0.9, beta_2=0.999)
        optimizer = Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999)  # TODO: settings of optimizer
        if (nb_gpus is None):
            ...
            p_model.compile(optimizer=optimizer,
                            loss=self.semi_supervised_loss(kidney),
