...
ENS_GT_PATH = '/data/suhita/temporal/skin/output/sm_mc/sadv111/'
num_epoch = 1000
batch_size = 8
DIM = [192, 256]
N_CHANNELS = 3
...
def train(gpu_id, nb_gpus, perc, batch_nos, learning_rate=None):
    ...
    TRAIN_NUM = len(np.load('/data/suhita/data/skin/Folds/train_fold' + str(FOLD_NUM) + '.npy'))
    NAME = 'sm_skin_entropy_F' + str(FOLD_NUM) + '_Perct_Labelled_' + str(perc)

    TB_LOG_DIR = '/data/suhita/data/tb/skin/' + NAME + '_' + str(learning_rate) + '/'
    MODEL_NAME = '/data/suhita/skin/' + NAME + '.h5'

    CSV_NAME = '/data/suhita/data/CSV/' + NAME + '.csv'

    TRAINED_MODEL_PATH = '/data/suhita/skin/softmax_supervised_sfs32_F_' + str(
        FOLD_NUM) + '_1000_5e-05_Perc_' + str(
        perc) + '_augm.h5'
...
'''
    ...
    val_fold = np.load('/data/suhita/data/skin/Folds/val_fold' + str(FOLD_NUM) + '.npy')
