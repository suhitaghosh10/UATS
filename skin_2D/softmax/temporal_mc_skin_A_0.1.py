...
DATA_PATH = '/cache/suhita/data/skin/softmax/fold_' + str(FOLD_NUM) + '_P' + str(PERCENTAGE_OF_LABELLED) + '/'
TRAIN_NUM = len(np.load('/cache/suhita/skin/Folds/train_fold' + str(FOLD_NUM) + '.npy'))
NAME = 'sm_skin_mc_F' + str(FOLD_NUM) + '_Perct_Labelled_' + str(PERCENTAGE_OF_LABELLED)
...
TRAINED_MODEL_PATH = '/cache/suhita/skin/models/softmax_supervised_sfs32_F_1_1000_5e-05_Perc_' + str(
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            if epoch > 0:
                ...
                for b_no in np.arange(num_batches):
                    ...
                    cur_pred[:, :, :, 0] = model_out[0] if bg_save else ensemble_prediction
                    cur_pred[:, :, :, 1] = model_out[1] if lesion_save else ensemble_prediction
