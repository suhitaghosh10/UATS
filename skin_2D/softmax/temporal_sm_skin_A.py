...
def train(gpu_id, nb_gpus, perc, batch_nos):
...
'''
    ...
    lcb = wm.LossCallback()
    es = EarlyStopping(monitor='val_skin_dice_coef', mode='max', verbose=1, patience=50)
...
if __name__ == '__main__':
    ...
    try:
        train(None, None, perc=0.05, batch_nos=5)
        shutil.rmtree(ENS_GT_PATH)
        train(None, None, perc=0.1, batch_nos=5)
        shutil.rmtree(ENS_GT_PATH)
        train(None, None, perc=0.25, batch_nos=5)
        shutil.rmtree(ENS_GT_PATH)
        train(None, None, perc=0.5, batch_nos=4)
        shutil.rmtree(ENS_GT_PATH)
        train(None, None, perc=1.0, batch_nos=3)
