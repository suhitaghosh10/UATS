...
def get_dice_from_array(arr1, arr2):
    # y_true_f = np.flatten(arr1)
    # y_pred_f = np.flatten(arr2)
    y_true_f = arr1
    y_pred_f = arr2
...
def evaluateFiles_arr(prediction, GT_array, csvName, connected_component= False):
    with open(csvName, 'w') as csvfile:
        ...
        for imgNumber in range(0, nrImgs):
            ...
            GT_label = GT_arr[imgNumber]

            if connected_component:
                print('Connected Component function not available')
            ...
            dice = get_dice_from_array(GT_label, pred_arr)
...
def create_test_arrays(test_dir):
    ...
    cases = sorted(os.listdir(test_dir+'/imgs'))

    # open first case to obtain dimensions
    segm = np.load(os.path.join(test_dir, 'imgs', cases[0]))
    DIM = segm.shape
    img_arr = np.zeros((len(cases), DIM[0], DIM[1], DIM[2]), dtype=float)
    GT_arr = np.zeros((len(cases), DIM[0], DIM[1], 1), dtype=float)

    for i in range(len(cases)):
        img_arr[i, :, :, :] = np.load(os.path.join(test_dir, 'imgs', cases[i]))
        GT_arr[i , :, :, :] = np.load(os.path.join(test_dir, 'GT', cases[i][:-4]+'_segmentation.npy'))[:,:,0:1]
...
if __name__ == '__main__':
    ...
    out_dir = '/home/anneke/projects/uats/code/skin_2D/output/'
    model_dir = out_dir +'/models'

    GT_dir = '/cache/anneke/skin/preprocessed/labelled/test'
    ...
    csvName =os.path.join(out_dir, 'evaluation', NAME+'.csv')
