    ...
    with open(csvName, 'w') as csvfile:
        ...
        for imgNumber in range(0, nrImgs):
            ...
            if eval:
                ...
                # GT_label.SetSpacing(pred_img.GetSpacing())

                # dice = getDice(pred_img, GT_label)
                dice = get_dice_from_array(pred_img_arr, GT_array[imgNumber,:,:,1])
...
def create_test_arrays(test_dir, eval=True):
    ...
    for i in range(len(cases)):
        if eval:
            GT_temp = np.load(os.path.join(test_dir, 'GT', cases[i]).replace('.npy', '_segmentation.npy'))
            GT_temp = GT_temp / 255
            GT = GT_temp[:, :, 0]
            del GT_temp
            GT_arr[i, :, :, 1] = GT
...
def eval_for_supervised(model_dir, img_path, model_name, eval=True, out_dir=None):
    if eval:
        ...
        GT_arr = None
    DIM = img_arr.shape
    from skin_2D.model_softmax_baseline import weighted_model
    wm = weighted_model()
    ...
    prediction = model.predict(img_arr, batch_size=1) # only foreground=skin for evaluation
    # os.makedirs(os.path.join(model_dir,'skin','evaluation'))
    csvName = os.path.join(model_dir[:-7], 'evaluation', model_name + '.csv')
