...
class weighted_model:
    ...
    def dice_coef(self, y_true, y_pred, smooth=1.):
        ...
        y_true_f = tf.where(tf.greater(y_true_f, 0.5), K.ones_like(y_true_f), K.zeros_like(y_true_f))
        y_pred_f = K.flatten(y_pred)
        y_pred_f = tf.where(tf.greater(y_pred_f, 0.5), K.ones_like(y_pred_f), K.zeros_like(y_pred_f))
        intersection = K.sum(y_true_f * y_pred_f)

        return (2. * intersection + smooth) / (K.sum(y_true_f) + K.sum(y_pred_f) + smooth)

    def dice_tb(self, input, class_wt=1., alpha=0.6):
        def dice_loss(y_true, y_pred, input=input, smooth=1., axis=(1, 2, 3)):
    ...
    def semi_supervised_loss_afs(self, input, unsup_loss_class_wt=1., alpha=0.6):
        ...
        if (nb_gpus is None):
            ...
            p_model.compile(optimizer=optimizer,
                            loss={'pz': self.semi_supervised_loss(pz, unsup_loss_class_wt=1),
                                  ...
                                  'afs': self.semi_supervised_loss_afs(afs, 1),
