...
def augment_images_spatial(original_image, reference_image, augmentation_type, T0, T_aug, transformation_parameters,
    ...
    if augmentation_type == AugmentTypes.FLIP_HORIZ.value:
        ...
        for current_parameters in transformation_parameters:
            ...
            aug_image = resampleToReference(aug_image, reference_image, interpolator, default_intensity_value)  ## is it really performing augmentation?
...
def get_augmentation_transform(img, reference_image, augmentation_type):
    ...
    translationsYZ_Arr = [np.random.uniform(-4, 4), np.random.uniform(-4, 4), 0.0, np.random.uniform(-4, 4),
                          np.random.uniform(-4, 4)]  # in mm
    translationsX_Arr = [np.random.uniform(-6, 6), np.random.uniform(-6, 6), 0.0, np.random.uniform(-6, 6),
    ...
    if AugmentTypes.ROTATE.value == augmentation_type:
        ...
        transl_x = translationsX_Arr[rand_vec[3]]
        transl_y = translationsYZ_Arr[rand_vec[4]]
        transl_z = translationsYZ_Arr[rand_vec[5]]
        scale = 1.
