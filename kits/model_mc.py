...
class weighted_model:
    ...
    def build_model(self, img_shape=(32, 168, 168), learning_rate=5e-5, gpu_id=None,
        ...
        unsupervised_label = Input((*img_shape, 1), name='unsup_label_inp')
        supervised_flag = Input((*img_shape, 1), name='flag_inp')
        ...
        conv_out_sm = Conv3D(1, (1, 1, 1), activation='sigmoid', name='conv_final_softmax')(conv7)

        kidney = K.stack([unsupervised_label, supervised_flag])
        ...
        conv_out_mc = Conv3D(1, (1, 1, 1), activation='sigmoid', name='conv_final_softmax_mc')(conv7)
        ...
        if (nb_gpus is None):
            merged_model.compile(optimizer=optimizer,
                                 loss=self.semi_supervised_loss(kidney, unsup_loss_class_wt=1)
                                 ,
                                 metrics=[self.dice_coef, self.unsup_dice_tb(kidney, 1), self.dice_tb(kidney, 1)]
            ...
            with tf.device(gpu_id):
                ...
                merged_model.compile(optimizer=optimizer,
                                     loss=self.semi_supervised_loss(kidney, unsup_loss_class_wt=1)
                                     ,
                                     metrics={'kidney': [self.dice_coef, self.unsup_dice_tb(kidney, 1),
                                                         self.dice_tb(kidney, 1)]
                                              }
                                     )
