...
TB_LOG_DIR = '/home/suhita/zonals/temporal/tb/variance_mcdropout/sl' + str(learning_rate) + '/'
MODEL_NAME = '/home/suhita/zonals/temporal/temporal_sl.h5'
...
# TRAINED_MODEL_PATH = '/home/suhita/zonals/temporal/temporal_mse.h5'
        ...
        def __init__(self, imgs_path, gt_path, ensemble_path, weight_path, supervised_flag,
            ...
            for patient in np.arange(num_train_data):
                np.save(self.weight_path + str(patient) + '.npy',
                        np.ones((32, 168, 168, NUM_CLASS)).astype('float32'))
            ...
            if epoch >= 0:
                ...
                for b_no in np.arange(num_batches):
                    ...
                    if b_no == 0:
                        ...
                        flag = np.where(np.reshape(np.max(cur_pred[num_labeled_train:actual_batch_size], axis=-1),
                                                   flag.shape) >= 0.99, np.ones_like(flag), flag)
                        self.supervised_flag[num_labeled_train:actual_batch_size] = flag
                        del flag
                    else:
                        flag = self.supervised_flag[start:end]
                        flag = np.where(np.reshape(np.max(cur_pred, axis=-1), flag.shape) >= 0.99, np.ones_like(flag),
                                        flag)
                    ...
                    if epoch >= SAVE_WTS_AFTR_EPOCH:
                        ...
                        unsupervised_weight = np.where(ensemble_prediction >= 0.99, np.zeros_like(ensemble_prediction),
                                                       np.ones_like(ensemble_prediction))  # consider hard labels
                        del ensemble_prediction
...
def predict(val_x_arr, val_y_arr):
    ...
    model = wm.build_model(num_class=NUM_CLASS, use_dice_cl=False, learning_rate=learning_rate, gpu_id=None,
                           nb_gpus=None, trained_model='/home/suhita/zonals/temporal/temporal_sl.h5')
    print('load_weights')
    # model.load_weights()
    print('predict')
